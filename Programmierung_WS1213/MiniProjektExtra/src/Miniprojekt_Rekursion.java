public class Miniprojekt_Rekursion {
	void allgemeinerTest(Datenverwaltung testobjekt) {
		...
		System.out.println("Es sind " + testobjekt.anzahlEintraege()
				+ " Eintraege vorhanden.");
		System.out.println("Es sind " + testobjekt.getAll(23).anzahlEintraege()
	...
	void binaerBaumTest(DatenBBaum testobjekt) {
		...
		for (int i = 0; i < test1.length; i++)
			test1[i] = new Beschreibung("",
					(int) (Math.random() * Integer.MAX_VALUE), "", i);
		testobjekt.array2baum(test1);
		System.out.println("Bei " + testobjekt.anzahlEintraege()
				+ " Eintraegen ist die Tiefe " + testobjekt.tiefe()
				+ " Ebenen.");
		...
		for (int i = 0; i < test2.length; i++)
			if (test2[i] == null)
				test2ArrayOk = false;
		System.out
				.println("Baum2Array liefert ein Array mit genausoviel Elementen wie der Baum enthalten sollte: "
						+ test2ArrayOk);

		testobjekt.baumBalancieren();
		System.out.println("Bei " + testobjekt.anzahlEintraege()
				+ " Eintraegen ist die Tiefe nach dem Balancieren "
				+ testobjekt.tiefe() + " Ebenen.");
	}
