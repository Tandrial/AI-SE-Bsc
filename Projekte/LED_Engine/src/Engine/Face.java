...
public class Face {
	...
	public double[][] toArray() {
		double[][] res = new double[4][pts.size()];
		int pos = 0;
		try {
			for (Vector3D p : pts) {
				res[0][pos] = p.x;
	...
	public void scale(double x_fak, double y_fak, double z_fak) {
		for (int i = 0; i < pts.size(); i++) {
			pts.set(i, Matrix.scale_M(x_fak, y_fak, z_fak).mult(pts.get(i)));
		}
	}

	public void trans(double x_off, double y_off, double z_off) {
		for (int i = 0; i < pts.size(); i++) {
			pts.set(i, Matrix.trans_M(x_off, y_off, z_off).mult(pts.get(i)));
		}
	}
	...
	public static List<Vector3D> line(Vector3D p1, Vector3D p2) {
		...
		do {
			res.add(new Vector3D(x0, y0, 0, Vector3D.VECTOR_POSITION));
			if (x0 == x1 && y0 == y1)
				break;
			int e2 = 2 * err;
			if (e2 > -dy) {
