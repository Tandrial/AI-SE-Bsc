...
void mcu_timer_start(MCU_TIMER_NUM num){	mcu_timer_set_start(num, true);		}
...
static USART_t *mcu_uart_usartxn[MCU_UART_TOTAL_NUMBER] = {&USARTC0, &USARTC1, &USARTD0, &USARTD1, &USARTE0, &USARTE1, &USARTF0, &USARTF1};
...
static const MCU_IO_PIN mcu_uart_tx_pin[MCU_UART_TOTAL_NUMBER] = {PC_3, PC_0, PD_3, PD_0, PE_3, PE_0, PF_3, PF_0};	/**< @todo Es wurde nicht ausprobiert ob alle IO Pins richtig sind, da dies im Datenblatt [1] nicht richtig angegeben war. */
