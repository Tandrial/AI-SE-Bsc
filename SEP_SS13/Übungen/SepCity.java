...
// ==========
{
	 ...
	 * @param Aufrufparameter - Parameter von der Kommandozeile
	 */
	...
	// --------------------------------
	{
		...
		Anfang(sepCity, "SepCity Musterlösung Einstiegsaufgabe",
				25, 2);
	}
	
	/**
	 * Druckt eine Hilfe zu SepCity auf der Kommandozeile. 
	 * Hinweis: Die Hilfe muss von Studenten während des Testats nicht erweitert werden.
	 */
	public void Hilfe() {
		System.out.println("--------------- Eingaben für SepCityOracle: -------------------");
		System.out.println("Stadt   <Name>                                           ");
		System.out.println("Weg     <Name>  <Tor> ,  <Zahl> ,  <Name>  <Tor>         ");
		System.out.println("Ausgabe                                                  ");
		System.out.println("direkt  <Name> ,  <Name>                                 ");
		System.out.println("Dreieck <Name> ,  <Name> ,  <Name>                       ");
		System.out.println("---------------------------------------------------------");
	}
	
	/**
	 * Diese Methode reagiert auf eine Eingabe, die aus der Eingabezeile
	 * (oder aus einer Zeile der Eingabe-Datei) gelesen wurde. Die aufge-
	 * rufene Methode eingegeben (...) liefert genau dann true, wenn die
	 * Eingabe dem Muster in dem String-Parameter genau entspricht. Darin
	 * bedeuten:
	 * N Eingabe eines neuen Städtenamens, der noch nicht existiert.
	 * S Eingabe eines bereits zuvor eingegebenen Städtenamens.
	 * T Himmelsrichtung eines Tors - möglich sind: n w s o.
	 * L Eingabe einer natürlichen Zahl, um eine Länge in km anzugeben.
	 * Wenn auf ein Kommandowort, z.B. "Stadt", eine syntaktisch abwein-
	 * chende Eingabe folgt, wird in die String-Variable Fehlermeldung ein
	 * entsprechender Text eingetragen und false zurückgegeben.
	 * @param Kommando - Das Kommando, auf das SepCity reagieren soll.
	 */
	...
	{		
		...
		if (eingegeben(Kommando, "Stadt N"))
			...
			neuerWeg(Wort(Kommando, 1, 2), Wort(Kommando, 1, 3).charAt(0), liesInt(Kommando, 2, 1),
					Wort(Kommando, 3, 1), Wort(Kommando, 3, 2).charAt(0));
			...
			dreieck(Wort(Kommando, 1, 2), Wort(Kommando, 2, 1), Wort(Kommando, 3, 1));
			...
			Königsreise(Wort(Kommando, 1, 2), Wort(Kommando, 2, 1));
			...
			System.out.println("*** Unzulässiges Kommando: " + Wort(Kommando, 1, 1) + " ***");
	...
	/**
	 * Prüft, ob die Eingabe dem Muster entspricht, das als String-Para-
	 * meter übergeben wurde (wie in der Methode Antwort erläutert).
	 * @param Kommando - Das eingegebene Kommando.
	 * @param SollEingabe - Das Muster, dem die Eingabe entsprechen soll.
	 */
	boolean eingegeben(String Kommando, String SollEingabe)
	{
		...
		for (int i = 1; i <= SatzAnz(Kommando); i++) { // Prüfe, ob im Satz i die Anzahl
			...
			for (int j = 1; j <= WortAnz(Kommando, i); j++) { // Soll ist das laut
													// SollEingabe geforderte
													// Wort.
													// Ist ist das an der
													// betreffenden Stelle
													// eingegebene Wort.
				String Soll = Wort(SollEingabe, i, j), Ist = Wort(Kommando, i, j);
				//
				// Falls gefordert wird, den Namen einer neuen Stadt einzugeben,
				// prüfe, ob die Eingabe einen solchen enthält. Namen müssen mit
				...
				// enthält.
				// Innerhalb einer Eingabezeile darf ein Städtenamen nicht mehr-
				// fach vorkommen:
				...
				// einzugeben,
				// prüfe, ob die Eingabe eine solche enthält. Zulässige Bezeich-
				...
				// Falls gefordert wird, eine Länge in km einzugeben, prüfe, ob
				// die
	 ...
	 * @param TorZeichen - Der erste Buchstabe der Himmelsichtung des gesuchten
     * Tores.
	...
	/**
	 * Sucht den Index eines Tores im Weg-Array der Stadt.
	 * @param s - Die Stadt zu der das gesuchte Tor gehört.
	 * @param w - Der Weg, der von dem gesuchten Tor ausgeht.
	 * @return Der Index des gesuchten Tores (bezogen auf das Weg-Array der
	 * Stadt.
	 */
	...
	/**
	 * Gibt den Namen zu einem Tor auf Basis seines Indizes zurück.
	 * Kann z.B. zur Ausgabe von Toren in einer lesbaren Form genutzt
	 * werden.
	 * @param TorIndex - Der Index des Tores bezüglich des Stadt-Arrays
	 * der Stadt zu der das Tor gehört.
	...
	/**
	 * Sucht die Nachbarstadt, die mit einer Stadt über einen Weg an
	 * einem Tor verbunden ist.
	 * @param Ausgangsstadt - Die Stadt, von der aus gesucht werden soll.
	 * @param Ausganggstor - Das Tor, von dem aus gesucht werden soll.
	 * @return null, wenn entweder die Ausgangsstadt null ist, oder wenn
	 * kein Weg vom Ausgangstor ausgeht. Sonst die Stadt, die über den
	 * Weg am Ausgangstor mit der Ausgangsstadt verbunden ist.
	 */
	 ...
	 * @param St - Der Name der neu anzulegenden Stadt.
	 */
	...
	/**
	 * Legt einen neuen Weg in Sepanien an und fügt diesen zu seinen
	 * Ausgangs- und Zielstädten hinzu.
	 * Hinweis: Da Wege keine Richtung haben, hat ein Vertauschen von
	 * Ausgangs- und Zielstadt keine Auswirkungen.
	 * @param St1 - Der Name der Ausgangsstadt des Weges.
	 * @param Tor1 - Der Name des Tores der Ausgangsstadt des Weges,
	 * an dem der Weg beginnt.
	 * @param L - die Länge des Weges.
	 * @param St1 - Der Name der Zielstadt des Weges.
	 * @param Tor1 - Der Name des Tores der Zielstadt des Weges,
	 ...
	 * @param St1 - Der Name der ersten Stadt.
	 * @param St2 - Der Name der zweiten Stadt.
	 */
	void direkterWeg(String St1, String St2) {
		...
		if (w != null) {
			...
			System.out.println("Kein direkter Weg zwischen " + St1 + " und " + St2);
		}
	}

	/**
	 * Prüft, ob ein direkter Weg zwischen zwei Städten vorliegt und gibt einen 
	 * solchen zurück.
	 * @param St1 - Name der ersten Stadt
	 * @param St2 - Name der zweiten Stadt
	 * @return Falls ein direkter Weg vorliegt, wird dieser zurückgegeben. Sonst
	 * gibt die Methode null zurück, um anzuzeigen, dass keine direkte Verbindung
	 * existiert.
	 */
    ...
    /**
	 * Löst die Aufgabe "Dreieck" aus der Übung. Prüft also, ob zwischen jedem
	 * Paar aus einer Menge von drei Städten ein direkter Weg existiert. Falls 
	 * ein solches Dreieck existiert, wird dieses ausgegeben, andernfalls wird 
	 * ausgegeben, dass kein Dreieck existiert.
	 * @param St1 - Der Name der ersten Stadt aus der zu prüfenden Menge.
	 * @param St2 - Der Name der zweiten Stadt aus der zu prüfenden Menge.
	 * @param St3 - Der Name der dritten Stadt aus der zu prüfenden Menge.
	 */
	void dreieck(String St1, String St2, String St3) {
		...
		if ((w12 != null) && (w13 != null) && (w23 != null)) {
			...
			System.out.println("Weg von " + St1 + ", " + TorName(TorIndex(wo(St1), w12))
					+ " nach " + St2 + ", " + TorName(TorIndex(wo(St2), w12))
					+ " Länge " + w12.Laenge);
			System.out.println("Weg von " + St2 + ", " + TorName(TorIndex(wo(St2), w23))
					+ " nach " + St3 + ", " + TorName(TorIndex(wo(St3), w23))
					+ " Länge " + w23.Laenge);
			System.out.println("Weg von " + St3 + ", " + TorName(TorIndex(wo(St3), w13))
					+ " nach " + St1 + ", " + TorName(TorIndex(wo(St1), w13))
					+ " Länge " + w13.Laenge);
					

		} else {
			System.out.println(
					"Keine direkte Verbindung zwischen allen drei Städten!"
					);
		}
	...
	String abgelegen ()
    { 
	    String abgelegeneStadt = null;
		
		/* Tragen Sie hier Ihre Lösung zur 
		   Beispieltestaufgabe "abgelegen" ein. */
		
		return abgelegeneStadt;
  	}  	
