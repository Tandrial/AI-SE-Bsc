		...
		for(; pSetting <= ANT_TRANSMIT_POWER_0DBM; ++pSetting) {
			...
			while (true) {
				...
				while(true) {
					s = ANT_RecvPacket_Blockfree(buffer, BC_ANT_BUFFSIZE);
					if (s == RS_PACKET_COMPLETE) {
						if (buffer[2] == ANT_CHANNEL_EVENT && buffer[5] == ANT_MESSAGE_EVENT_RX_SEARCH_TIMEOUT) {
							if (state == DS_DECREASING) {
								printf("Still no connection (current range %fm! Decrease distance by 0.4m and press return!", distance);
							}else {
								state = DS_DECREASING;
								printf("Lost connection (current range %fm)! Decrease distance by 0.4m and press return!", distance);
							}
							distance -= .4f;
							break;
							...
							if (state == DS_DECREASING) {
								...
								printf("Still in range (current %fm)!\nIncrease distance by 0.4m and press return!", distance);
								distance += .4f;
							}
							break;
						}
