...
/**
 * Move a given area. Three children: x shearing, y shearing, filler
 *
 * @author Thomas Weise
 */
public class Movement extends GraphicsNode {
   ...
   */
  public Movement(final Node<?>[] pchildren,
      final NodeType<Movement, RealFunction> in) {
  ...
  public double compute(final GraphicsContext gc) {
    ...
    g = gc.getGraphics();
    at = g.getTransform();
    g.shear(Math.max(-1d, Math.min(1d, sh1)),//
        Math.max(-1d, Math.min(1d, sh2)));
  ...
  public void fillInText(final StringBuilder sb) {
    sb.append("move(dx="); //$NON-NLS-1$
    this.get(0).fillInText(sb);
    sb.append(",dy="); //$NON-NLS-1$
