 ...
 * @author Thomas Weise
 */
public class Floor extends RealFunction {
  ...
  public Floor(final Node<?>[] pchildren,
      final NodeType<Floor, RealFunction> in) {
    ...
    if ((a.hasConstantValue()) && (!(a.hasEffect()))) {
      ...
      if (q != null) {
        if ((allowed == null) || (allowed.containsType(q))) {
          d = Math.floor(a.getConstantValue());
          if (Double.isNaN(d)) {
            d = 0d;
          }
