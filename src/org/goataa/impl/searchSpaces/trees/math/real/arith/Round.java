 ...
 * @author Thomas Weise
 */
public class Round extends RealFunction {
  ...
  public Round(final Node<?>[] pchildren,
      final NodeType<Round, RealFunction> in) {
    super(pchildren, in, false);
  }

  /**
   * Compute the rounded value of a given number
   *
   * @param data
    ...
    if ((a.hasConstantValue()) && (!(a.hasEffect()))) {
      ...
      if (q != null) {
        if ((allowed == null) || (allowed.containsType(q))) {
          d = Math.rint(a.getConstantValue());
          if (Double.isNaN(d)) {
            d = 0d;
          }
