...
public class Cos extends RealFunction {
   ...
   */
  public Cos(final Node<?>[] pchildren,
      final NodeType<Cos, RealFunction> in) {
    ...
    if ((a.hasConstantValue()) && (!(a.hasEffect()))) {
      ...
      if (q != null) {
        if ((allowed == null) || (allowed.containsType(q))) {
          return q.instantiate(Math.cos(a.getConstantValue()));
        }
      }
    }

    if (a != oa) {
      return new Cos(new Node[] { a }, t);
    }
